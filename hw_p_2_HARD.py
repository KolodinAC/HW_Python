...
# Видел что строкой решать нельзя, но меня хватли только на такой вариант :)
...
try:
    if is_int(number) == True:
        ...
        x = number.split('.')     # разбил строку на 2 части и создал список разделенный на точкой
        a = int(x[0])             # перевел первый и второй элементы списка в int
        b = int(x[1])
        sum2 = 0
        while a != 0:
            sum2 = sum2 + a % 10
